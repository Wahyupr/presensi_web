...
<table width="100%" border="0" >
  <tr>
  <td  width="16%" height="178"><img src="<?php echo base_url(); ?>/assets/img/brand/akakom.png" width="200" height="100" /> </td >
    <td width="84%"><H2><p align="center"><strong>LAPORAN DATA PRESENSI SISWA PKL <br />
    </strong><strong>STMIK AKAKOM YOGYAKARTA</strong></p>
    </H2>
      <div align="center">Alamat : Jl. Janti No.143, Jaranan, Banguntapan, Kec. Banguntapan, Bantul, Daerah Istimewa Yogyakarta 55198
      </p>
<P>No.Telp : +62 274 48666 Email : info@akakom.ac.id</p>
        </p>
    </div></td>
  </tr>
</table>
...
<table width="100%" border="0" class="saya">
                ...
                <td class="style5"><div align="center"><?php echo $d['nis']; ?> <?php  $id_jurusan=$d['id_jurusan']; ?></div></td>
   ...
   $no=1;
                                                        ...
                                                        $query=$this->db->query("Select count(*) as hadir from kehadiran join user on user.id_user=kehadiran.id_user join sekolah on sekolah.id_sekolah=user.id_sekolah where user.id_jurusan = '$id_jurusan' and status='Hadir' and tgl_absensi BETWEEN '$awal' AND '$akhir'");
   ...
   $no=1;
                                                        ...
                                                        $query=$this->db->query("Select count(*) as ijin from kehadiran join user on user.id_user=kehadiran.id_user join sekolah on sekolah.id_sekolah=user.id_sekolah where user.id_jurusan = '$id_jurusan' and status='Ijin' and tgl_absensi BETWEEN '$awal' AND '$akhir'");
                                                        foreach ($query->result() as $row): ?>
                                                       <?php  $ijin=$row->ijin; ?>
                                                        <?php endforeach; ?>
                                                        <p />
